...
#					(!($value1->{text} =~ /^ ?RT/)) || ($value1->{text} =~ /^ ?RT.*( |\b|\#)(epidemic|outbreak|plague|surge|increase|rise|alert|watch|warning)/i)
		...
		my $sth2 = $dbh->prepare("UPDATE `tweets` SET `match` = ?  where `tid` = ?;");
		$sth2->execute($value1->{match}, $results->{tid}) or die $dbh->errstr;
			} else {
		             	print "NOMATCH: ".$value1->{text};
				print "\n";
			}
...
#	 	if (($value1->{user}->{location} =~ /\,/)){
		...
		$value1->{coordinates}->{latitude} = $value1->{coordinates}->{coordinates}[1]||"";
		$value1->{coordinates}->{longitude} = $value1->{coordinates}->{coordinates}[0]||"";
		my $loc2parse = $value1->{user}->{full_name}?$value1->{user}->{full_name}:$value1->{user}->{location};
...
#					$logme = 1;
		...
		if(($value1->{user}->{location} =~ /^.+T:\s([234][0-9]\.\d+)\,(-\d{2,3}\.\d+)$/)
		    &&($1<=49 && $1 >=24 && $2<=-66 && $2>=-160)){
				$value1->{coordinates}->{latitude}= $1;
				$value1->{coordinates}->{longitude}= $2;
		} else {
			$value1->{coordinates}->{latitude}= 0;
			$value1->{coordinates}->{longitude}= 0;
		}
